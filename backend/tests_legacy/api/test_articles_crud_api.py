...
def test_article_create_succeeds_for_admin(client, db_session):
...
def test_article_create_missing_category_returns_400(client):
    _set_override(require_admin, lambda: {"email": "test@example.com"})
    try:
        payload = {
            "title": "New Article",
...
def test_article_create_invalid_slug_returns_422(client):
    _set_override(require_admin, lambda: {"email": "test@example.com"})
    try:
        payload = {
            "title": "New Article",
...
def test_article_update_succeeds_and_sets_published_at(client, db_session):
...
def test_article_update_not_found_returns_404(client):
    _set_override(require_admin, lambda: {"email": "test@example.com"})
    try:
        res = client.patch("/api/articles/999", json={"title": "Updated"})
    finally:
        _clear_override(require_admin)
...
def test_article_delete_succeeds_for_admin(client, db_session):
    category = _create_category(db_session)
...
def test_article_delete_not_found_returns_404(client):
    _set_override(require_admin, lambda: {"email": "test@example.com"})
    try:
        res = client.delete("/api/articles/999")
    finally:
        _clear_override(require_admin)
...
def test_list_articles_limit_validation_error(client):
    res = client.get("/api/articles?limit=51")
    assert res.status_code == status.HTTP_422_UNPROCESSABLE_CONTENT
    body = res.json()
    assert body["error"]["code"] == "REQUEST_VALIDATION_ERROR"


def test_list_articles_rejects_invalid_token(client):
    res = client.get("/api/articles", headers={"Authorization": "Bearer invalid"})
