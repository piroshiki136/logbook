...
## 現在のデプロイ状況メモ（2026-05-01 時点）
- Vercel へのデプロイ作業は進行中
- 本番用の環境変数は未投入のものがあり、順次追加している
- 公開 URL は `https://logbook-flame.vercel.app` で確定した
- 本番 DB は Neon を採用予定だが、まだ作成していない
- 本番用 JWT 鍵（`JWT_PUBLIC_KEY` / `JWT_PRIVATE_KEY`、必要に応じて frontend assertion 用鍵）も未作成
- `CORS_ALLOW_ORIGINS` は未設定だが、設定値は `https://logbook-flame.vercel.app` で確定した
