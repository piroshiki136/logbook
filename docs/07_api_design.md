...
## エラーハンドリング方針
- このプロジェクトでは、仕様上想定される業務エラーを `AppError` で表現する。TODO や設計メモで `DomainError` と呼んでいるものは、現行実装では `AppError` が該当する。
- `AppError` と `HTTPException` は、共通エラー形式 `{ "error": { "code": string, "message": string } }` で返す。
- `400` / `401` / `403` / `404` はユーザーに表示可能な想定内エラーとして扱い、本番でも `message` を返す。
- 代表例:
  - `400`: 入力内容や参照先が仕様に合わない。例: `カテゴリが見つかりません`
  - `401`: 認証に失敗した。例: `認証に失敗しました`
  - `403`: 管理権限がない。例: `権限がありません`
  - `404`: 対象リソースが存在しない。例: `記事が見つかりません`
- `422` のリクエストバリデーションエラーは `REQUEST_VALIDATION_ERROR` として返す。本番では詳細を丸め、`入力内容が正しくありません` を基本文言にする。
- フロントエンドは API の `error.message` をそのままフォームや画面内エラーに表示してよい。ただし、一覧取得など復旧方法が明確でない画面では、画面ごとの汎用文言（例: `記事一覧の取得に失敗しました。しばらくしてから再度お試しください。`）を表示する。
- 記事詳細の `404` は Next.js の `notFound()` に変換し、記事詳細専用の 404 画面を表示する。
- 本番では `APP_MODE=prod` を設定し、FastAPI の `debug=false` で動作させる。
- DB エラーや未処理例外の詳細、スタックトレース、接続文字列などの内部情報はレスポンスに含めない。未処理例外は `INTERNAL_SERVER_ERROR` と固定文言 `現在エラーが発生しています` を返し、詳細はアプリケーションログでのみ確認する。
- Next.js のエラー画面も `error.message` や stack を表示せず、利用者向けの固定文言と復旧導線だけを表示する。
- 初期リリースでは管理画面内にログ閲覧 UI は提供しない。500 エラーの詳細確認は Vercel の Runtime Logs / Function Logs を利用する。
...
# 9. レートリミット方針
- MVP では全体的な共有レートリミットは導入しない。
- 公開 API は読み取り中心とし、記事作成・更新・削除・画像アップロードなどの書き込み API は管理者認証で保護する。
- 管理者は少人数運用を前提とし、通常運用ではアクセス量が限定的である。
- DB / ホスティングの従量課金リスクが低いため、初期リリースでは Vercel / backend のログ確認で運用する。
- 異常なアクセスや負荷が確認された場合のみ、Vercel 側の保護機能または Redis 等を使った共有レートリミットを検討する。
- `/api/auth/token` は認証交換の連打を抑えるため、既存のアプリ内簡易レートリミットを維持する。
...
# レートリミット（目安）
- MVP: 全体的な共有レートリミットは導入しない。公開 API は読み取り中心とし、書き込み API は管理者認証で保護する。
- 認証交換: `/api/auth/token` のアプリ内簡易レートリミットは既存実装として維持する。
- 運用: 初期リリースでは Vercel / backend のログ確認で運用する。
- 将来対応: 異常なアクセスや負荷が確認された場合のみ、Vercel 側の保護機能または Redis 等の共有ストアを用いたアプリ内制御を導入する。
