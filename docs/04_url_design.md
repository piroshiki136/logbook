...
- NextAuth の `basePath` は `/api/auth`。`AUTH_URL` はローカル `http://localhost:3000/api/auth`、本番 `https://<project>.vercel.app/api/auth` に統一する
- GitHub OAuth App の Authorization callback URL は本番 `https://<project>.vercel.app/api/auth/callback/github`、ローカル `http://localhost:3000/api/auth/callback/github` を登録する
