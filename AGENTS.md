...
Use Biome defaults (2-space indent, double quotes, trailing commas) and PascalCase filenames for React components, camelCase for hooks, kebab-case for slugs as stored in `articles.slug`. Tailwind classes should be grouped layout → spacing → color to stay consistent with `docs/06`. Python relies on Ruff for lint/format, with 4-space indents, snake_case functions, PascalCase Pydantic schemas, and double-quoted strings. Use SQLAlchemy 2.0 style APIs consistently. Keep API JSON fields camelCase (`createdAt`, `isDraft`) even though DB columns are snake_case per `docs/05`.
